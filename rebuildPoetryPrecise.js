...
// Add to handleAuthLogin
content = content.replace(
  'setFavorites(user.favorites || []);',
  'setFavorites(user.favorites || []);\n    setFavoritePoems(user.favoritePoems || []);'
);

// Add to refreshCurrentUser
content = content.replace(
  'setFavorites([]);',
  'setFavorites([]);\n      setFavoritePoems([]);'
);
...
// Add POETRY to Desktop Nav precisely
const dtNavTarget = `onClick={() => navigateTo("about")}`;
// eslint-disable-next-line no-unused-vars
const dtNavClassName = `className={\`px-3 py-1.5 rounded font-semibold text-sm hover:bg-[#4E1A27] hover:text-[#FFD59F] transition \${activeTab === "about" ? "bg-[#4E1A27] text-[#FFD59F]" : ""}\`}`;

const newDtNav = `<button
...
const desktopReplaceTarget = `<button
            onClick={() => navigateTo("about")}`;

content = content.replace(desktopReplaceTarget, newDtNav + '\n            ' + dtNavTarget.slice(8));

// Add POETRY to Mobile Nav precisely
...
content = content.replace(mobileNavTarget, newMobileNav);

// Replace Hero Buttons
const heroStart = `<div className="pt-6 sm:pt-8 flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center relative z-20 w-full sm:w-auto">`;
const heroEnd = `Hindi/Urdu Books
                  </button>
                </div>`;
...
if (heroStartIndex !== -1 && heroEndIndex !== -1) {
  ...
  content = content.slice(0, heroStartIndex) + newHeroButton + content.slice(heroEndIndex + heroEnd.length);
}
...
const favStart = `{activeTab === "favorites" && (`;
const favEnd = `</section>
        )}`;
const favStartIndex = content.indexOf(favStart);
// find next instance of favEnd after favStartIndex
const favEndIndex = content.indexOf(favEnd, favStartIndex);

if (favStartIndex !== -1 && favEndIndex !== -1) {
const newFavoritesJSX = `{activeTab === "favorites" && (
  ...
  content = content.slice(0, favStartIndex) + newFavoritesJSX + content.slice(favEndIndex + favEnd.length);
}
